defmodule OpenSauceWeb.PortalLive.Invoice do
  ...
  def render(assigns) do
    ~H"""
    <div style={"font-family:'Hanken Grotesk',system-ui,sans-serif;-webkit-font-smoothing:antialiased;padding-bottom:60px;min-height:100dvh;color:#{@brand.text};background:#{@brand.bg};--s-bg:#{@brand.bg};--s-paper:#{@brand.paper};--s-border:#{BrandTheme.rgba(@brand.border, 0.58)};--s-text:#{@brand.text};--s-muted:#{@brand.muted};--s-dim:#{@brand.dim};"}>
      <%!-- minimal top bar --%>
      <div style="padding:16px 16px 10px;display:flex;align-items:center;justify-content:space-between;">
        ...
        <span
          ...
          style={"background:#{BrandTheme.rgba(@accent, 0.15)};color:#{@accent};border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700;"}
        >
          Paid
        </span>
      </div>
        ...
        <div style="background:var(--s-paper,#211E16);border:1px solid var(--s-border,rgba(52,48,37,0.58));border-radius:20px;overflow:hidden;">
          ...
          <div style="padding:16px 20px;border-bottom:1px solid var(--s-border,rgba(52,48,37,0.58));display:flex;gap:20px;align-items:flex-start;">
            ...
            <div style="flex:1;min-width:0;">
              ...
              <div
                :if={addr = @display_customer && billing_address_of(@display_customer)}
                style="margin-top:4px;"
              >
                <p :if={addr.street} style="font-size:12px;color:var(--s-muted,#9A9384);">
                  {addr.street}
                </p>
          ...
          <div style="padding:0 20px 20px;">
            ...
            >
              ...
              <span style="font-size:12px;color:var(--s-muted,#9A9384);font-variant-numeric:tabular-nums;">
                {fmt_money(@display_org.currency, @invoice.amount)}
              </span>
            </div>
            <div
              :for={tax <- @tax_lines}
            ...
            >
              ...
              <span style="font-size:12px;color:var(--s-muted,#9A9384);font-variant-numeric:tabular-nums;">
                {fmt_money(@display_org.currency, tax.amount)}
              </span>
            </div>
            <div style="display:flex;justify-content:space-between;align-items:baseline;padding-top:10px;margin-top:4px;border-top:1px solid var(--s-border,rgba(52,48,37,0.58));">
              <span style="font-size:14px;font-weight:700;color:var(--s-text,#F4EFE2);">Total</span>
              <span style={"font-family:'Bricolage Grotesque',sans-serif;font-size:22px;font-weight:700;color:#{@accent};letter-spacing:-0.02em;font-variant-numeric:tabular-nums;"}>
  ...
  defp compute_taxes(subtotal, tax_rates, :exclusive) do
    ...
    {tax_lines, subtotal_d |> D.add(tax_total) |> D.round(2)}
  end
    ...
    {groups_map, group_order, ungrouped} =
      Enum.reduce(items, {%{}, [], []}, fn item, {map, order, flat} ->
        case item["group_id"] do
          ...
          gid ->
            {Map.update(map, gid, [item], &(&1 ++ [item])), if(gid in order, do: order, else: order ++ [gid]), flat}
        end
      end)
