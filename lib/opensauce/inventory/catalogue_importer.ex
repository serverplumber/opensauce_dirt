defmodule OpenSauce.Inventory.CatalogueImporter do
  @moduledoc """
  Imports supplier catalogues from PDF files using Claude.
  ...
  @prompt """
  Extract every product from this nursery supplier catalogue PDF.
  ...
  - "format_description"  — size/grade/format verbatim from catalogue (e.g. "tige, 100mm WB", "Pots 1 gallon (3 L)", "30L bag") (string or null)
  ...
  - Do not invent data. If a field is not in the catalogue, use null.
  ...
  @doc """
  Writes a previewed item list to the database under the given catalogue.
  Returns {:ok, %{created: n, failed: n}}.
  """
  def commit(items, supplier_catalogue_id, opts \\ []) when is_list(items) do
    ...
    results =
      Enum.map(items, fn item ->
        ...
        OpenSauce.Inventory.create_supplier_catalogue_item(attrs, actor: actor, tenant: tenant)
      end)
